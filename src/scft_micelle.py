...
import optax
import os
from .scft_utils_micelle import simpsons_weights, scft, scftRefine, evaluateFields, sequence_to_identity, evaluateEnergyInverse
...
class SCFTMicelle:
    def __init__(self, space_group, sequence, c, solvent):
        ...
        self.c = c # Polymer fraction of the solution
    ...
    def initialGuess(self, f_basis, sg_list, initial_star_dict, strength):
        ...
        for sg in sg_list:
            ...
            solvent_psi = jnp.absolute(field_initialize(Ntaus, 1)) # Initial solvent field is random number centered around 1-c
            # Transform a few times to get the right zeroed values
            solvent_star = jnp.matmul(FT, solvent_psi)
            solvent_star = solvent_star.at[0].set(1 - self.c)
            solvent_psi = jnp.matmul(FS, solvent_star)
            # Positive only
            solvent_psi = jnp.absolute(solvent_psi)
            ...
            for j, combo in enumerate(combinations):
                ...
                # Normalize the psi_field to be mean c
                psi_star = jnp.matmul(FT, psi_field)
                psi_star = psi_star.at[0].set(self.c)
                psi_field = jnp.matmul(FS, psi_star)
                # Set to be positive
                psi_field = jnp.absolute(psi_field)
    ...
    def convergePhase(
        ...
        # a solvent chemical identity is required to be specified which generates the solvent chi
        ...
        x_opt = {"xi": x["xi"], "psi": x["psi"], "solvent_psi": x["solvent_psi"]}
        cell = x["cell"]
        ...
        for epoch in range(epoch_param["first_optimization"]):
            ...
            opt_state, x_opt, loss_value, aux = step_LowEnergy(
                opt_state, x_opt, cell, angle, solvent, f_basis
            )
            (
                w_matlab,
                w_report,
        ...
        if angle.size > 0:
            ...
            x = {"xi": xi, "psi": x_opt["psi"], "solvent_psi": x_opt["solvent_psi"], "cell": cell, "angle": angle}
        else:
            xi = self.shiftXi(x_opt["xi"], sg)
            x = {"xi": xi, "psi": x_opt["psi"], "solvent_psi": x_opt["solvent_psi"], "cell": cell}
        # Return the converged phase
        return x

    def refinePhase(self, x, solvent, f_basis, sg, lr_param, epoch_param):
        ...
        def step(
            ...
            output, grads = jax.value_and_grad(scftRefine, has_aux=True)(
                x, cell, angle, c, solvent, f_basis, space_group, u0_list, dt, Ns, stencil, basis
            )
            loss_value, aux = output
    ...
    def run_scft(self, sg: str, load : bool = False, load_dict : dict = None, save : bool = False):
        ...
        if load:
            ...
            initial_star_dict = {sg: self.space_group[sg].initial_star} # This is a dictionary, just for consistency
            x_dict, sg_dict = self.initialGuess(f_basis, [sg], initial_star_dict, 0.02) # 0.1
        ...
        if self.space_group[sg].unit_cell == 'lam':
            dim = 1
        elif self.space_group[sg].unit_cell == 'hex' or self.space_group[sg].unit_cell == 'square' or self.space_group[sg].unit_cell == 'rectangular' or self.space_group[sg].unit_cell == 'oblique':
            dim = 2
        else:
            dim = 3
        ...
        for struct_name, sg in sg_dict.items():
            ...
            # Evaluate the phase
            x_eval = {"f_basis": f_basis, "solvent": self.solvent} | x_sg[struct_name]
