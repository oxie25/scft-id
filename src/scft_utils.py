...
from .utils import nested_checkpoint_scan, CHECKPOINT_FLAG, NESTED_LENGTHS, GLOBAL_NS
...
def strang_splitting_scan(w, u_forward, u_backward, FT, FS, GsqStar, stencil):
    ...
    def strang(state, potential_half_t):
        ...
        u_step_tau_backward = (
            potential_half_t[:, 1] * u_step_tau_backward
        )  # what we want
    ...
    if CHECKPOINT_FLAG:
        _, final_state = nested_checkpoint_scan(
            strang, initial_state, potential_half, nested_lengths=NESTED_LENGTHS
        )
    else:
...
def hamiltonian(Q, psi, w, L_ss, dt, FT, FS):
...
def scft(
    x, cell, angle, f_basis, space_group, u0_list, reg_param, dt, Ns, stencil, basis_fun
):
    ...
    w_pred = jnp.einsum("ijk,jr->ik", L_psi, stencil) - xi
    ...
    incompress = integral(psi_hat, stencil) - 1  # Integrate over s and subtract 1
    ...
    w_matlab = jnp.sqrt(
        jnp.sum((psi_star[1:, :] - psi_hat_star[1:, :]) ** 2)
        / jnp.sum(psi_hat_star[1:, :] ** 2)
    )
    ...
    F = hamiltonian(Q_hat, psi_hat, w_pred, L_ss, dt, FT, FS)  # - incompress_energy[0]

    # Disorder energy loss term
    F_disorder = hamiltonian(
        1, jnp.ones_like(psi_hat), jnp.zeros_like(w_pred), L_ss, dt, FT, FS
    )
    ...
    for i in range(dRbasis.shape[0]):
        ...
        Rs = int_u_prod_star.flatten() * dGsq * Gwt**2

        stress_d = 1 / 6 * 1 / Q_hat * 1 / V * jnp.sum(Rs)
...
def scftRefine(
    x, cell, angle, f_basis, space_group, u0_list, dt, Ns, stencil, basis_fun
):
    # Extract the required parameters
    xi = x["xi"]
    psi = x["psi"]
    ...
    # Regularize the zeroth wavevector to be 1 - we can do this on the calculated psi, and this should guide the guessed psi into the correct answer
    psi_hat_star = psi_hat_star.at[0, :].set(1.0)
    ...
    scft_loss = (
        w_loss + incompressible_loss + psi_loss + xi_loss
...
def scftID(x, phi_target, space_group, u0_list, reg_param, dt, Ns, stencil, basis_fun):
    ...
    phi_dev_loss = jnp.mean((phi_target - phi_hat) ** 2)
    ...
    energy_loss = F  # Just the free energy
...
def evaluateEnergyInverse(x, space_group, u0_list, dt, Ns, stencil, basis_fun):
    ...
    # Energy of current structure accounting for not satisfying incompressibility
    # Don't need incompressible energy, use the calculated density field structure
    F = hamiltonian(Q_hat, psi_hat, w_pred, L_ss, dt, FT, FS)
...
def evaluateFields(x, space_group, u0_list, dt, Ns, stencil, basis_fun):
